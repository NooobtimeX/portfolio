                    ...
                    {groupedExperiences.map((group, groupIndex) => (
                        ...
                        >
                            ...
                            <div className="absolute left-6 md:left-1/2 transform -translate-x-1/2 w-16 h-16 bg-primary border-4 border-white shadow-[0_0_25px_theme(colors.primary.DEFAULT)] z-20 flex items-center justify-center rounded-full text-white">
                                <span className="font-[Bangers] text-xl">{groupedExperiences.length - groupIndex}</span>
                            </div>

                            <div className="md:grid md:grid-cols-2 md:gap-20 pl-24 md:pl-0">

                                {/* Company Info Panel */}
                                <div className={`mb-8 md:mb-0 ${groupIndex % 2 === 0 ? "md:text-right" : "md:col-start-2 md:text-left"}`}>
                                    <div className={`inline-block bg-white p-6 border-4 border-black shadow-[10px_10px_0px_0px_rgba(255,50,50,1)] hover:scale-105 transition-transform ${groupIndex % 2 === 0 ? "md:mr-8" : "md:ml-8"} relative`}>
                                        {/* Speech Bubble Tail for Company Info */}
                                        <div className={`absolute top-1/2 -translate-y-1/2 w-6 h-6 bg-white border-l-4 border-b-4 border-black transform rotate-45 z-[-1] ${groupIndex % 2 === 0 ? "right-[-14px] border-r-0 border-t-0" : "left-[-14px] border-l-2 border-b-2"}`}></div>

                                        <div className={`flex items-center gap-6 ${groupIndex % 2 === 0 ? "md:flex-row-reverse" : ""}`}>
                                            {group.logo && (
                                                <Image
                                                    src={group.logo}
                                            ...
                                            <div>
                                                <h3 className="font-[Bangers] text-3xl uppercase tracking-wide text-black leading-none mb-1">{group.company}</h3>
                                                <div className={`flex items-center gap-1 ${groupIndex % 2 === 0 ? "md:justify-end" : "justify-start"}`}>
                                                    <Icon icon="material-symbols:location-on" className="text-primary w-5 h-5" />
                                                    <span className="font-[Inter] text-black font-bold uppercase text-sm">{group.location}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                >
                                    ...
                                    <div className={`hidden md:block absolute top-8 ${groupIndex % 2 === 0 ? "right-[-5rem] w-20" : "left-[-5rem] w-20"} h-2 bg-white border-y-2 border-black z-0`}></div>
                                    ...
                                    {group.experiences.map((experience, expIdx) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center gap-3 text-gray-300 font-[Inter] mb-6 bg-black/40 p-3 -mx-2 rounded border border-white/10">
                                                ...
                                                <span className="uppercase text-sm font-bold tracking-wider text-primary">{experience.type}</span>
                                            </div>
